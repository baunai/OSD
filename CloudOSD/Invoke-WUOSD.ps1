...
Write-Log -Message "Run Windows Updates of $OSName build version $OSVer"

$Updates = Get-WindowsUpdate -MicrosoftUpdate -NotCategory "Drivers" -AcceptAll
...
if ($null -eq $wuServer) {
    ...
    foreach ($Update in $Updates) {
        Write-Log -Message "KB: $($Update.KB)"
        Write-Log -Message "Title: $($Update.Title)"
        Write-Log -Message "Description: $($Update.Description)"
        Write-Log -Message "URL: $($Update.SupportUrl)"
        Write-Log -Message "Size: $($Update.Size)"
        Write-Log -Message "Initiating installation of KB: $($Update.KB)"
        $OutputText = Install-WindowsUpdate -ComputerName "$env:COMPUTERNAME" -AcceptAll -ForceInstall -IgnoreReboot -MicrosoftUpdate -KBArticle $($Update.KB) -Verbose *>&1 | Out-String
        Write-Log -Message $OutputText
        Write-Log -Message "Finished installing KB: $($Update.KB)"
        Write-Log -Message "-----------------------------------------------------------"       
    }    
}
...
if ($null -ne $wuServer) {
    ...
    Restart-Service wuauserv
    Write-Log -Message "Complete Windows Wpdates on $env:COMPUTERNAME"   
}
